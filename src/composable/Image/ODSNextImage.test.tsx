...
describe('ODSNextImage 컴포넌트', () => {
  ...
  Object.entries(imageSizes).forEach(([sizeToken, { width, height }]) => {
    it(`${sizeToken} 사이즈에 대한 width와 height가 올바르게 설정되어야 한다`, () => {
      const { getByAltText } = render(
        <ODSNextImage {...testProps} sizeToken={sizeToken as ImageSizeToken} />,
      );
      const image = getByAltText(testProps.alt) as HTMLImageElement;

      expect(image).toHaveAttribute('width', String(calcRemToPxNumber(width)));
